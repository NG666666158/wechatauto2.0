...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Out-of-process emergency stop watchdog.")
...
def _process_alive(pid: int) -> bool:
    ...
    if sys.platform.startswith("win"):
        result = subprocess.run(["tasklist", "/FI", f"PID eq {pid}"], check=False, capture_output=True, text=True)
        ...
        fallback = subprocess.run(
            ["powershell", "-NoProfile", "-Command", f"$p = Get-Process -Id {pid} -ErrorAction SilentlyContinue; if ($p) {{ 'RUNNING' }}"],
            check=False,
            capture_output=True,
            text=True,
        )
