...
def _normalize_dedupe_part(value: object) -> str:
    return normalize_dedupe_part(value)
...
class WeChatAIApp:
    ...
    def _normalize_group_sender_name(self, chat_id: str, sender_name: str | None) -> str:
        return normalize_group_sender_name(chat_id, sender_name)
    ...
    def _order_unread_message_records(
        ...
        return order_unread_message_records(records, contexts)
    ...
    def _message_dedupe_signature(
        ...
        *,
        ...
        return message_dedupe_signature(
    ...
    def _legacy_unread_text_signature(self, session_name: str, text: str) -> str:
        return legacy_unread_text_signature(session_name, text)
    ...
    def _flush_active_pending(
        ...
        *,
        ...
        result = {"friend_replies": 0, "group_replies": 0, "errors": 0}
        if pending_state_is_empty(self.active_pending_session, self.active_pending_messages):
            return result

        if not should_flush_active_pending(
            active_pending_session=self.active_pending_session,
            active_pending_messages=self.active_pending_messages,
            now=now,
            current_session_name=current_session_name,
            active_pending_deadline=self.active_pending_deadline,
            force=force,
        ):
            return result
